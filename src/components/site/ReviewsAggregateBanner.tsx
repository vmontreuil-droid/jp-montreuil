...
import type { ReviewWithPhoto } from '@/lib/shop/reviews'

/**
 * Sociaal-bewijs banner voor de homepage. Toont:
 *  - Overall sterren + gemiddelde + count
 *  - 1 wisselende quote (random gekozen uit recent approved)
 *  - Link naar /[locale]/avis (verschijnt enkel wanneer count >= 10)
 *
 * Drempel: minimum 3 reviews om enige indicatie te geven; daaronder
 * verbergt het component zich volledig (return null) zodat een
 * eenmalige slechte rating niet je homepage drukt.
 */
...
export function ReviewsAggregateBanner({
  ...
  locale: 'fr' | 'nl'
  labels: {
    eyebrow: string       // "Avis vérifiés" / "Geverifieerde beoordelingen"
    basedOn: string       // "D'après {{n}} avis" / "Gebaseerd op {{n}} beoordelingen"
    seeAll: string        // "Voir tous les avis" / "Alle beoordelingen bekijken"
    on: string            // "sur" / "op"
  }
}) {
  if (aggregate.count < MIN_REVIEWS_TO_SHOW || aggregate.average == null) return null
  // Pak een (semi-)random quote — server-render dus puur seedless OK
  const candidates = recentReviews.filter((r) => r.body && r.body.trim().length > 0)
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 md:py-14 grid md:grid-cols-[auto_1fr] gap-8 md:gap-12 items-center">
        ...
        {quote && (
          <div className="flex flex-col gap-3">
            ...
            <blockquote className="text-base md:text-lg text-(--color-ink) leading-relaxed italic">
            ...
            {showPageLink && (
              <Link
                ...
                className="mt-2 inline-flex items-center gap-1 text-xs uppercase tracking-[0.2em] text-(--color-bronze) hover:text-(--color-bronze-dark) transition-colors self-start"
              >
                {labels.seeAll} →
              </Link>
            )}
