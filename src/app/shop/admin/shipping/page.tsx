...
export default async function ShopShippingAdminPage() {
  ...
  if (!user) redirect('/admin/login?next=/shop/admin/shipping')
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-10 space-y-6">
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-stone-900">Frais de port</span>
      </div>
      ...
      {zones.length === 0 ? (
        <p className="text-stone-500">Geen zones — voeg er minstens 1 toe.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {zones.map((z) => {
                ...
                return (
                  <tr key={z.id} className="border-b border-stone-100">
                    <td className="p-2">
                      <form id={`z-${z.id}`} action={upd} className="contents">
                        <input type="text" name="name" defaultValue={z.name} required
                          className="w-full px-2 py-1 bg-white border border-stone-300 rounded text-sm" />
                      </form>
                    </td>
                    <td className="p-2">
                      <input form={`z-${z.id}`} type="text" name="countries"
                        ...
                        className="w-full px-2 py-1 bg-white border border-stone-300 rounded text-xs font-mono" />
                    </td>
                    <td className="p-2">
                    ...
                    <td className="p-2 flex gap-1 justify-end">
                      <button form={`z-${z.id}`} type="submit"
                        className="px-3 py-1 bg-stone-900 text-white text-xs rounded hover:bg-stone-800">
                        ✓
                      </button>
                      <form action={deleteZone.bind(null, z.id)}>
                        <button type="submit" className="p-1.5 text-stone-400 hover:text-amber-700">
                          <Trash2 size={14} />
                        </button>
                      </form>
                    </td>
