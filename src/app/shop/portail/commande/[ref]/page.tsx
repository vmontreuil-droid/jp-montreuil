...
export default async function ShopOrderTrackingPage({
    ...
    return (
      <main className="max-w-md mx-auto px-6 py-16 text-center space-y-4">
        ...
        <p className="text-sm text-stone-600">
          Pour consulter cette commande, ajoutez votre e-mail à l&apos;URL :
        </p>
        <code className="block bg-stone-100 p-2 rounded text-xs break-all">
          /shop/portail/commande/{ref}?email=votre@email.com
        </code>
        <p className="text-xs text-stone-500">
          Le mail de confirmation contient ce lien complet.
        </p>
      </main>
    )
  }
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <header>
        <p className="text-xs text-stone-500 uppercase tracking-widest mb-1">Référence</p>
        <h1 className="text-3xl font-mono">{order.reference}</h1>
        <p className="text-sm text-stone-600 mt-2">
          Commandé le {new Date(order.created_at).toLocaleDateString('fr-BE')}
        </p>
