...
export default async function AboutPage({ params }: Props) {
  ...
  const sections = data ?? []
  const ibooks = await getActiveIbooks()
        ...
        <section className="max-w-5xl mx-auto px-6 pb-24 space-y-10">
          <header className="text-center">
            <p className="text-xs uppercase tracking-[0.2em] text-(--color-bronze) mb-3 inline-flex items-center gap-2 justify-center">
              ...
              {locale === 'fr' ? 'Les livres' : 'De boeken'}
            </p>
          </header>

          {ibooks.map((b) => {
            const title = locale === 'fr' ? b.title_fr : b.title_nl
            const description = locale === 'fr' ? b.description_fr : b.description_nl
            const fallbackTitle = title || b.title_fr || b.title_nl
            return (
              <div
                key={b.id}
                className="bg-(--color-paper) border border-(--color-frame) p-6 md:p-8"
              >
                {fallbackTitle && (
                ...
                {b.pdf_path && (
                  <div className="mt-6 flex flex-wrap justify-center gap-3 pt-6 border-t border-(--color-frame)">
                    <IbookViewer
                      ...
                      title={fallbackTitle || (locale === 'fr' ? 'Le livre' : 'Het boek')}
